...
if reload_matlab_files
    ...
    p, cdid, obsw = vecF64.([v["p"], v["data"]["cdid"], v["data"]["obsweight"]])
    α, β          = Float64.([v["alpha"][1], v["beta"]])

    f1_mat, f2_mat, f3_mat, sum2_mat, expU_mat = vecF64.([v["f1"],
...
if reload_matlab_files
    ...
    Dm, D0, dDm, dD0, γ0, p, l1_m = vecF64.([v["Dm"], v["D0"], v["dDm"], v["dD0"],
                                             v["gamma0"], v["p"], v["l1"]])
    r    = Float64(v["r"])
    γ1_m = vecC64(v["gamma1"])
...
γ1, l1 = solve_γ(Dm, D0, dDm, dD0, γ0, p, r; allout = true)
...
if reload_matlab_files
    ...
    cdindex, d_first  = vecI64.([v["data"]["cdindex"], v["data"]["first"]])
...
######################
if reload_matlab_files
    v_in = matread("$path/obscal_inputs.mat")
    βσ3, basellh, p0 = vecF64.([v_in["betasigma3"],
         v_in["basellh"], v_in["p0"]])
    data     = v_in["data"]
    rounderr = Float64(v_in["rounderr"])

    v_mat = Dict{String,Any}()
...
for (i,j) in [(0,0),(0,1)] #,(0,1),(1,1)]
    demandcal = (i == 1)
    WFcal     = (j == 1)
    lip, γ2, γ1, γ0, D0, Dm, pi_v, CSns, CSs = obscalnewtest2015(βσ3, data, basellh, p0, rounderr;
